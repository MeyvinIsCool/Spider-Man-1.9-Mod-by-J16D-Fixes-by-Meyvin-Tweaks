...
LVAR_INT iCheckpoint[6] iBlip[5]
...
READ_INT_FROM_INI_FILE "CLEO\SpiderJ16D\config.ini" "config" "LANG" (iLanguage)   // 0:SPA ||1:ENG
    ...
    IF IS_PLAYER_PLAYING player
        GOSUB draw_timer
        //tw_a3 - cx[4] cy[4] cz[4] czAngle[4]	|| czAngle[9]
        x[1] = cx[4]
        y[1] = cy[4]
        z[1] = cz[4]
        czAngle[9] = czAngle[4]
            ...
            IF LOCATE_CHAR_ANY_MEANS_3D player_actor (x[0] y[0] z[0]) (10.0 10.0 10.0) FALSE 
                ...
                AND NOT IS_BUTTON_PRESSED PAD1 CIRCLE           // ~k~~PED_FIREWEAPON~   
                    ...
                    SET_CHAR_HEADING player_actor zAngle
                        zAngle = czAngle[4]	
                        GOSUB animSequence
                    GOTO reset_vars_to_checkpointF
                ENDIF
            ELSE
                x[0] = x[1]  
                y[0] = y[1] + 9.982
    ...
    IF IS_PLAYER_PLAYING player
        GOSUB draw_timer
        //tw_a3 - cx[5] cy[5] cz[5] czAngle[5]	|| czAngle[10]
        x[1] = cx[5]
        y[1] = cy[5]
        z[1] = cz[5]
        czAngle[10] = czAngle[5]
        czAngle[10] -= 180.0
        //-- Coords
        x[0] = x[1] 
        y[0] = y[1] + 9.982
            ...
            IF LOCATE_CHAR_ANY_MEANS_3D player_actor (x[0] y[0] z[0]) (10.0 10.0 10.0) FALSE 
                ...
                AND NOT IS_BUTTON_PRESSED PAD1 CIRCLE           // ~k~~PED_FIREWEAPON~   
                    ...
                    SET_CHAR_HEADING player_actor zAngle
                        zAngle = czAngle[5]	
                        GOSUB animSequence
                    GOTO reset_vars_to_checkpointG
                ENDIF
            ELSE
                x[0] = x[1]  
                y[0] = y[1] - 9.982
    ...
    ENDIF
        WAIT 0
    GOTO checkpointF_loop         

    //Seventh    (6)
    reset_vars_to_checkpointG:
    iPlusScore =# fTotalScore
    iMissionScore += iPlusScore
    ...
    IF IS_PLAYER_PLAYING player
        GOSUB draw_timer
        //tw_a4 - cx[6] cy[6] cz[6] czAngle[6]	|| czAngle[11]
        x[1] = cx[6]
        y[1] = cy[6]
        z[1] = cz[6]
        czAngle[6] = czAngle[11]
        czAngle[6] -= 180.0
        //-- Coords
        x[0] = x[1] 
        y[0] = y[1] + 9.982        
...
mission_failed:
    IF DOES_FILE_EXIST "CLEO\SpiderJ16D\sp_prt.cs"
	    STREAM_CUSTOM_SCRIPT "SpiderJ16D\sp_prt.cs" 3    
        WAIT 2000
    ENDIF
...
mission_passed:
    ...
    GET_CLEO_SHARED_VAR varScrewBallProgress (counter)
    counter ++
    SET_CLEO_SHARED_VAR varScrewBallProgress counter
    WRITE_INT_TO_INI_FILE counter "CLEO\SpiderJ16D\config.ini" "stadistics" "sp_screwb"  
    SET_PLAYER_JUMP_BUTTON player FALSE
    WHILE TRUE  
        GOSUB draw_total_score
        GOSUB draw_key_press
...
barFunc:
    ...
    var[0] = xSize
    var[0] /= 2.0
    var[0] += 177.0                                // 177.0 - 1280 x 720  //192.5 - 1440 x 900 224.0 - 1980 x 1080
