...
clear_all_mission_files:
    USE_TEXT_COMMANDS FALSE
    flag_player_on_mission = 0  //0:not in mission
...
prepare_wave_deal:
    ...
    WHILE number_of_members >= iCounter
        ...
        CLEO_CALL get_random_coords 0 (xIn yIn zIn) 37.0 (x[0] y[0] z[0])
        GET_NTH_CLOSEST_CAR_NODE (x[0] y[0] z[0]) 7 (x[1] y[1] z[1])
        ...
        CREATE_CHAR PEDTYPE_MISSION1 randomVal (x[1] y[1] z[1]) (iChar)
            ...
            SET_CHAR_HEALTH iChar 100
            SET_CHAR_SHOOT_RATE iChar 30
            SET_CHAR_ACCURACY iChar 30
            CLEO_CALL set_char_mass 0 iChar fPedMass  //default=70.0
            SET_FOLLOW_NODE_THRESHOLD_DISTANCE iChar 25.0     //Sets the range within which the char responds to events
            SET_INFORM_RESPECTED_FRIENDS iChar 15.0 2     //gossip
...
load_all_needed_models:
    ...
    REQUEST_MODEL 2919  // kmb_holdall
    LOAD_ALL_MODELS_NOW
RETURN
...
remove_loaded_models:
    ...
    MARK_MODEL_AS_NO_LONGER_NEEDED 2919  // kmb_holdall
RETURN
...
save_map_coords:
    ...
    IF DOES_FILE_EXIST "cleo\SpiderJ16D\config.ini"
        ...
        STRING_FORMAT (_lName)"drug%i" id_coords
        READ_STRING_FROM_INI_FILE "cleo\SpiderJ16D\config.ini" "drdl" $_lName (iVar)
